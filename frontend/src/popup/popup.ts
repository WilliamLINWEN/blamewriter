...
class PopupController {
  ...
  private renderLLMModelSelection(): void {
    ...
    if (provider && provider.models && provider.models.length > 0) {
      ...
      provider.models.forEach(model => {
        ...
        this.llmModelSelect.appendChild(o);
      });
      if (
        this.currentUserLLMConfig &&
      ...
      const o = document.createElement('option');
      o.value = '';
      o.textContent = selectedProviderId ? '-- No Models --' : '-- Select Provider --';
      this.llmModelSelect.appendChild(o);
    }
  }
  ...
  private async loadAndRenderTemplates(): Promise<void> {
    ...
    try {
      ...
      if (this.availableTemplates.length === 0) {
        const o = document.createElement('option');
        o.value = '';
        o.textContent = '-- No Templates Configured --';
        this.templateSelect.appendChild(o);
        this.templateSelect.disabled = true;
      } else {
        const dO = document.createElement('option');
        dO.value = '';
        dO.textContent = '-- Select a Template --';
        this.templateSelect.appendChild(dO);
        this.availableTemplates.forEach(t => {
          const o = document.createElement('option');
          o.value = t.id;
  ...
  private async checkCurrentPage(): Promise<void> {
    try {
      ...
      if (!prInfo) {
        this.showError('Not a Bitbucket PR page.');
  ...
  private validateTokenInput(): void {
    ...
    this.generateButton.disabled =
      !isValid || !templateSelected || !providerSelected || !modelSelected;

    if (tokenValue.length > 0) {
      this.tokenInput.classList.toggle('invalid', !isValid);
      if (!isValid && this.statusMessage.textContent === '') {
        // Only show token error if no other error is shown
        this.showError('Invalid Bitbucket OAuth token format.');
      } else if (isValid && this.statusMessage.className.includes('error')) {
        // Clear if valid and error was shown
  ...
  private async handleGenerateClick(): Promise<void> {
    try {
      ...
      if (this.generateButton.disabled) {
        ...
        } else if (!this.templateSelect.value) {
          this.showError('Please select a template.');
        } else if (!this.llmProviderSelect.value) {
          this.showError('Please select an LLM Provider.');
        } else if (!this.llmModelSelect.value) {
          this.showError('Please select an LLM Model.');
        } else {
          this.showError('Please ensure all fields and selections are valid.');
        }
        return;
      }
      // @ts-ignore
      ...
      if (!selectedTemplate) {
        ...
        return;
      } // Should be caught by button state
      ...
      if (
        ...
        this.showError('LLM configuration error. Please save settings in Options page.');
        return;
      }
      ...
      this.showError('Unexpected error during generation.');
    } finally {
    ...
    return new Promise(resolve => {
      ...
      chrome.runtime.sendMessage(request, (response: GenerateResponse) => {
        ...
        if (chrome.runtime.lastError) {
          resolve({ error: 'Background script error.' });
        } else {
  ...
  private async copyToClipboard(): Promise<void> {
    ...
    try {
      ...
      this.showSuccess('Copied (fallback)!');
    }
  }
  ...
  private setLoadingState(loading: boolean): void {
    const commonDisableCondition =
      !this.isValidToken(this.tokenInput.value.trim()) ||
  ...
  private showError(message: string): void {
    if (this.statusMessage) {
      this.statusMessage.textContent = String(message || 'An error occurred');
