...
import {
  ...
  TemplateUsageStats,
  Template,
} from './storage_schema'; // To allow type hints for keys
    ...
    try {
      ...
      chrome.storage.sync.get([key], result => {
        ...
        if (chrome.runtime.lastError) {
          // @ts-ignore
          console.error(
            `Error getting item '${key}' from storage:`,
...
export async function saveToStorage(
  ...
  if (checkQuota) {
    ...
    if (!canSave) {
      throw new Error(
        'Storage quota exceeded and cleanup failed. Please free up space or reduce data size.',
      );
    }
  }
 ...
 */
export async function checkStorageQuotaBeforeSave(
  items: Partial<ExtensionStorage>,
): Promise<boolean> {
  try {
    ...
    console.log(
      `Storage check: Current ${quotaInfo.usagePercentage}%, Estimated after save: ${Math.round(estimatedUsagePercent)}%`,
    );
 ...
 */
// async function saveToStorageWithQuotaCheck(
//   items: Partial<ExtensionStorage>,
...
export async function performStorageCleanup(): Promise<boolean> {
  try {
    ...
    const remainingUsageStats = usageStats.filter(
      s => !templatesToRemove.some(tr => tr.id === s.templateId),
    );
