...
function validateGenerateRequest(body: any): { isValid: boolean; errors: ValidationError[] } {
  ...
  if (!body.prUrl) {
    ...
    errors.push({
      field: 'prUrl',
      message:
        'Invalid Bitbucket PR URL format. Expected format: https://bitbucket.org/workspace/repo/pull-requests/id',
    });
  }
 ...
 */
function createSuccessResponse(description: string): GenerateMVPResponse {
  return {
    description,
  };
}
...
async function handleGenerateMVP(req: express.Request, res: express.Response): Promise<void> {
  ...
  try {
    ...
    const successResponse = createSuccessResponse(generatedDescription);
